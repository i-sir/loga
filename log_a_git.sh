...
clear_directory() {
    # 指定要清空的文件夹路径
    directory_to_clear="/www/wwwroot/log-a/git_log/"
...
create_folder_and_file() {
    ...
    # 指定要创建的文件夹路径和文件路径
    folder_to_create="/www/wwwroot/log-a/git_log/"
...
git_commit() {
    # 指定Git仓库路径和主分支名称
    git_repo_path="/www/wwwroot/log-a"
    main_branch="master"
    ...
    # 提交到Git仓库
    git add --all
    git commit -m "更新--[log-a];$commit_message"
